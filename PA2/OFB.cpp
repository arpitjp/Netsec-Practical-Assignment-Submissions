...
string OFBencrypt(string plaintext)
{
    ...
    for (int i = 0; i < blocks; i++)
    {
        if (i == 0)
        {
            ...
            string iv = textToHex("MohanCHE"); /**IV with 8char=16hex=64bit done**/
            block_iv = blowfishencrypt(iv);
            output_cipher = xorofb(block_iv, plaintext_block);
            hex_plaintext.erase(0, s);
        }
        else
        {
            string plaintext_block = hex_plaintext.substr(0, s);
            block_iv = blowfishencrypt(block_iv);
    ...
    cout << "\nOutput Cipher Text by OBC Encrypt: " << output_cipher << endl;
    cout << " " << endl;
