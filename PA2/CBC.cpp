...
vector<vector<string>> S = {{"d1310ba6", "98dfb5ac", "2ffd72db", "d01adfb7", "b8e1afed",
...
string blowfishdecrypt(string ciphertext)
{
    string key = "aabb09182736ccdd";
...
string BEFirst(string plaintext)
{
    string iv = textToHex("chemical");
...
string BDFirst(string ciphertext)
{
    string iv = textToHex("chemical");
    string plain = blowfishdecrypt(ciphertext);
    string outputPT = xorcbc(iv, plain);
    return outputPT;
}

string BDMid(string cipheriv, string ciphertext)
{
    string plain = blowfishdecrypt(ciphertext);
    string outputPT = xorcbc(cipheriv, plain);
    return outputPT;
}
...
string CBCencrypt(string plaintext)
{
    ...
    for (int i = 0; i < blocks; i++)
    {
        if (i == 0 && hex_plaintext.size() >= 16)
        {
            ...
            cipher_iv = BEFirst(hex_plaintext.substr(0, 16));
